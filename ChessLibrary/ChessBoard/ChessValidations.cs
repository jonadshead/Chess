...
namespace Chess
{
    public partial class ChessBoard
    {
        ...
        internal static bool IsValidMove(Move move, ChessBoard board, bool raise, bool checkTurn)
        {
            ...
            {
                // Capture
                if (!(board.pieces[move.NewPosition.Y, move.NewPosition.X] is null)
        ...
        internal static bool IsKingCheckedValidation(Move move, PieceColor side, ChessBoard board)
        {
            ...
            if (move.Parameter is MoveCastle castle && move.Piece.Color == side
             && !(move.Piece is null)) // move.Piece is null only when calling recursively
            {
                ...
                for (short i = kingPos.X; i < 7 && i > 1; i += step)
                    if (IsKingCheckedValidation(new Move(kingPos, new Position() { Y = kingPos.Y, X = i }), side, board))
                        return true;
            ...
            else if (move.OriginalPosition != move.NewPosition)
            {
                ...
                fboard.DropPieceToNewPosition(new Move(move.ToString()));
            }
        ...
        private static bool IsKingChecked(PieceColor side, ChessBoard board)
        {
            ...
            for (short i = 0; i < 8; i++)
            {
                for (short j = 0; j < 8; j++)
                {
                    if (!(board.pieces[i, j] is null) && board.pieces[i, j].Color != side)
                    {
        ...
        private static bool RookValidation(Move move, Piece?[,] pieces)
        {
            ...
            if (v == 0 || h == 0)
            {
                ...
                for (int i = move.OriginalPosition.Y + stepV, j = move.OriginalPosition.X + stepH;
                    ...
                    i += stepV, j += stepH)
                {
                    if (!(pieces[i, j] is null) || (i == move.NewPosition.Y && j == move.NewPosition.X))
                    {
        ...
        internal static bool HasRightToCastle(PieceColor side, CastleType castleType, ChessBoard board)
        {
            ...
            bool ValidByMoves()
            {
                Position kingpos = new Position(side == PieceColor.White ? "e1" : "e8");
                ...
                return !(board.pieces[rookpos.Y, rookpos.X] is null)
        ...
        private static bool IsValidEnPassant(Move move, ChessBoard board, short v, short h)
        {
            if (Math.Abs(v) == 1 && Math.Abs(h) == 1)
            {
                ...
                if (!(piece is null) && piece.Color != move.Piece.Color && piece.Type == PieceType.Pawn)
                {
                    bool valid = false;
        ...
        internal static Position LastMoveEnPassantPosition(ChessBoard board)
        {
            ...
            if (!(lastMove is null) && lastMove.Piece.Type == PieceType.Pawn)
            {
                if (Math.Abs(lastMove.NewPosition.Y - lastMove.OriginalPosition.Y) == 2)
                {
                    pos = new Position() { X = lastMove.NewPosition.X, Y = (short)((lastMove.NewPosition.Y + lastMove.OriginalPosition.Y) / 2) };
                }
            }
